...
class Category extends Backend
{
    ...
    public function _initialize()
    {
		...
		// 1.1.9升级 排序方式
		if($type){
			$this->tree->init(collection($this->model->where('type', $type)->order('weigh asc,id asc')->select())->toArray(), 'pid');
		}else{
			$this->tree->init(collection($this->model->order('weigh asc,id asc')->select())->toArray(), 'pid');
		}
