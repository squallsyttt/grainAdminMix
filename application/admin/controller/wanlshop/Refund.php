			...
			if($row['type'] == 0){
				...
				if($config['config']['refund_switch'] == 'Y')
				{
					...
					if($wanlpay['code'] == 0)
					{
						...
						// 扣除本地用户余额
						controller('addons\wanlshop\library\WanlPay\WanlPay')->money(-$wanlpay['data']['money'], $wanlpay['data']['user_id'], '退款订单（订单号：'.$row->order->order_no.'）已原路返回余额至你的'.$wanlpay['data']['type_text'], 'sys');
